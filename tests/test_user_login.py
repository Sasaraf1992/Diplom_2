...
class TestUserLogin:
    ...
    def test_user_successful_auth(self, fake_user_required_login_password):
        email, password = fake_user_required_login_password
        lu = LoginUser()
        lu.user_login(data={'email': email, 'password': password})
        lu.check_status_code_and_text(200, 'success', True)

    @allure.title('Проверка ошибки авторизации пользователя при неверном логине')
    def test_user_auth_with_wrong_login(self, fake_user_required_login_password):
    ...
    def test_user_auth_with_wrong_password(self, fake_user_required_login_password):
